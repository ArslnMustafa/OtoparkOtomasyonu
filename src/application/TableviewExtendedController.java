...
public class TableviewExtendedController {
    ...
    void btn_guncelle_Click(ActionEvent event) {
    	...
    	if(kayitlar_table.getSelectionModel().getSelectedIndex()!=-1) {
    		...
    		//Yeni kayit
    		Kayitlar_X kk=new Kayitlar_X(idm+1, txt_kul.getText(), Double.parseDouble(txt_sifre.getText()), "Güncellenmiþ mesaj", butonlar[idm]);
    ...
    void initialize() {
    	...
    	//Tableview Veri girme
        	 ...
        	 veriler.add(new Kayitlar_X(2, "root", 0.89, "Mesaj 2", butonlar[butonNumarasi]));
        	 butonNumarasi++;
        	 veriler.add(new Kayitlar_X(3, "user", 0.56, "Mesaj 3", butonlar[butonNumarasi]));
        	 butonNumarasi++;
        	 veriler.add(new Kayitlar_X(4, "taze", 0.24, "Mesaj 4", butonlar[butonNumarasi]));
        	 butonNumarasi++; 
    ...
    void ButonOlayi(ActionEvent event) {
    	for (int i = 0; i < butonlar.length; i++) {
    		...
    		if(event.getSource()==butonlar[i]) {
    			for (Kayitlar_X islem: veriler) {
    				if(islem.getOnay().isSelected()) {
    					//									SÝKTÝM COK ONEMLI BURASI
    					int idm = islem.getId()-1;
    					Kayitlar_X  ort= new Kayitlar_X(islem.getId(),islem.getKul_adi(),Double.parseDouble(islem.getMetinkutusu().getText()),"1",butonlar[idm]);
    					int sira = kayitlar_table.getSelectionModel().getSelectedIndex();				
